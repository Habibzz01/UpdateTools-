...
class TerminalTheme:
  ...
  |_|  \_\___|\__|_| \_|\___|_|  \_\___/ \__|
        ...
        \033[1;31m⚡Developed Script Xbibz Official 👻🫦
        \033[0m
        """)
...
class BotNetTermux:
    ...
    def dapatkan_target(self):
        ...
        try:
            # Jika input bukan IP (ada domain)
            if not target.replace('.', '').isdigit():
                target_ip = socket.gethostbyname(target)
                print(f"\033[1;32m[+] Resolved IP: {target_ip}\033[0m")
                return target_ip
            return target
        except Exception as e:
            print(f"\033[1;31m[!] Gagal resolve target: {str(e)}\033[0m")
            return None
